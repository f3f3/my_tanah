  ...
  */
  class Tabulator
  {
    ...
    */
    ...
    {
      $this->start = $this->ci->input->post('page');
      $this->length = $this->ci->input->post('size');
      if(empty($this->start)){
        $this->start = $this->ci->input->get('page');
      }
      if(empty($this->length)){
        $this->length = $this->ci->input->get('size');
      }
      if(!empty($this->start) && !empty($this->length))
        $this->ci->db->limit($this->length, ($this->start-1)*$this->length);
    }
    /**
    ...
    */
    ...
    {
      $Data = $this->ci->input->post('sorters');
      if(empty($Data)){
        $Data = $this->ci->input->get('sorters');
      }
      if (!empty($Data)){
        foreach ($Data as $key=>$val){
          $this->ordered[$val["field"]]=$val['dir'];
          $this->ci->db->order_by($val["field"], $val['dir']);
        }
      }
    }
    /**
    ...
    */
    ...
    {
      $filters = !empty($this->ci->input->post('filters'))?$this->ci->input->post('filters'):[];
      foreach($filters as $val){
        $field = $this->select[$val['field']];
        if(!is_array($val['value'])){
          $role = (($val['type']!="=")?$val['type']:"where");
          if(!empty($this->header[$val['field']]["headerval"])){
            $value = array_search($val['value'],$this->header[$val['field']]["headerval"]);
          } else $value = $val['value'];
          $this->ci->db->{$role}($field,$value);
        } else {
          if(!empty($val['value']['start']))
            $this->ci->db->where($field." >=",$val['value']['start']);
          if(!empty($val['value']['end']))
            $this->ci->db->where($field." <=",$val['value']['end']);
        }
    ...
    */
    private function produce_output($output, $charset)
    {
      ...
      {
        if($istrue) {$this->columns=array_keys($row_val);$istrue = false;}
      ...
      if($output == 'json')
      {
        ...
        (
          ...
          'last_page' => ceil($iTotal/$this->length),
          'page'      => 5,
          'data'      => $aaData,
          'columns'   => $this->compile_header(),
          "sorters"   => $this->compile_ordered()
        );
        if($charset == 'utf-8')
        ...
        return array(
          'aaData'  => $aaData,
          'columns' => $this->compile_header(),
          'last_page' => $this->get_total_results()
        );
    }
    /**
    * Get result count
    ...
    */
    private function check_cType()
    {
      ...
      if(is_array($column) && is_numeric($column[0]['data']))
        return FALSE;
      else
        return TRUE;
    }
...
', '\b', '', '"'), array('\\', '\/', '\n', '\t', '\r', '\b', '\f', '"'));
    ...
    {
      if(empty($this->ci->input->post('sorters')))
        return $this->ci->db->order_by($a,$b);
    }
    ...
    /**
     * returns the sql statement of the last query run
     * @param costumize=[["orderable"=>bolean(true,false),"className"=>string(*any),"width"=>string(*num+px)]]
     */
    public function set_header($costumize)
    {
      $this->header = $costumize;
    }
    private function compile_ordered()
    {
      $result = [];
      foreach($this->ordered as $idx=>$val){
        $result[]=["column"=>$idx,"dir"=>$val];
      }
      return $result;
    }
    private function compile_header()
    {
      $result=[]; $arrayCol = [
        "title","field","visible","align","width","minWidth","widthGrow",
        "widthShrink","resizable","frozen","responsive","tooltip","cssClass",
        "rowHandle","hideInHtml","sorter","sorterParams","formatter","formatterParams",
        "variableHeight","editable","editor","editorParams","validator","mutator","mutatorParams",
        "mutatorData","mutatorDataParams","mutatorEdit","mutatorEditParams","mutatorClipboard",
        "mutatorClipboardParams","accessor","accessorParams","accessorData","accessorDataParams",
        "accessorDownload","accessorDownloadParams","accessorClipboard","accessorClipboardParams",
        "download","downloadTitle","topCalc","topCalcParams","topCalcFormatter","topCalcFormatterParams",
        "bottomCalc","bottomCalcParams","bottomCalcFormatter","bottomCalcFormatterParams",
        "cellClick","cellDblClick","cellContext","cellTap","cellDblTap","cellTapHold","cellEditing",
        "cellEdited","cellEditCancelled","headerSort","headerSortStartingDir","headerClick",
        "headerDblClick","headerContext","headerTap","headerDblTap","headerTapHold","headerTooltip",
        "editableTitle","titleFormatter","titleFormatterParams","headerFilter","headerFilterPlaceholder",
        "headerFilterParams","headerFilterEmptyCheck","headerFilterFunc","headerFilterFuncParams"
      ];
      foreach($this->columns as $idx=>$val){
        if(!in_array($val,$this->unset_columns)){
          $temp=["title"=>$val,"field"=>$val];
          if(!empty($this->header[$val])){
            foreach($this->header[$val] as $i=>$v){
              if(in_array($i,$arrayCol)){
                $temp[$i]=$v;
              }
            }
          }
          /*if(!empty($this->ordered[$val])){
            $temp["column"]=$val;
            $temp["dir"]=$this->ordered[$val];
          }*/
          $result[]=$temp;
        }
      }
      if(!empty($result) && $this->enableAction){
        $result[] = [
          "title"=>"Action",
          "field"=>"action",
          "formatter"=>"html",
...
/* Generated by Harviacode Codeigniter CRUD Generator 2019-06-16 17:00:01 */
/* http://harviacode.com */